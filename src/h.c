...
// added by module `builtin`, file: builtin_d_gcboehm.c.v:117:
...
VV_LOCAL_SYMBOL  int compare_16615326395714869970_RepIndex_by_idx(RepIndex* a, RepIndex* b) {
...
VV_LOCAL_SYMBOL  int compare_3722393162571173239_string(string* a, string* b) {
...
static int v_typeof_interface_idx_IError(int sidx) { /* IError */ 
	...
	if (sidx == _IError_os__Eof_index) return 120;
	if (sidx == _IError_os__NotExpected_index) return 121;
	if (sidx == _IError_os__FileNotOpenedError_index) return 123;
	if (sidx == _IError_os__SizeOfTypeIs0Error_index) return 124;
	if (sidx == _IError_os__ExecutableNotFoundError_index) return 142;
	return 30;
}
...
u64 math__bits__rem_64(u64 hi, u64 lo, u64 y) {
	multi_return_u64_u64 mr_16748 = math__bits__div_64((u64)(hi % y), lo, y);
	u64 rem = mr_16748.arg1;
	return rem;
}
...
VV_LOCAL_SYMBOL void array_push_many_noscan(array* a3, voidptr val, int size) {
	...
	if (a3->data == val && a3->data != 0) {
		...
		vmemcpy(array_get_unsafe(*a3, a3->len), copy.data, (u64)(((u64)(a3->element_size)) * ((u64)(size))));
	} else {
		array_ensure_cap_noscan(a3, (int)(a3->len + size));
		if (a3->data != 0 && val != 0) {
			vmemcpy(array_get_unsafe(*a3, a3->len), val, (u64)(((u64)(a3->element_size)) * ((u64)(size))));
		}
	}
	a3->len = ((int)(new_len));
}
...
VNORETURN VV_LOCAL_SYMBOL void panic_debug(int line_no, string file, string mod, string fn_name, string s) {
	...
	#else
	{
		...
		eprintln( str_intp(2, _MOV((StrIntpData[]){{_SLIT("   v hash: "), 0xfe10, {.d_s = _SLIT("ddb6685")}}, {_SLIT0, 0, { .d_c = 0 }}})));
		eprintln(_SLIT("========================================="));
		#if defined(_VNATIVE)
		{
			exit(1);
...
VV_LOCAL_SYMBOL void map_set(map* m, voidptr key, voidptr value) {
	...
	multi_return_u32_u32 mr_11690 = map_key_to_index(m, key);
	u32 index = mr_11690.arg0;
	u32 meta = mr_11690.arg1;
	multi_return_u32_u32 mr_11726 = map_meta_less(m, index, meta);
	index = mr_11726.arg0;
	meta = mr_11726.arg1;
	for (;;) {
		if (!(meta == m->metas[index])) break;
...
void map_reserve(map* m, u32 meta_bytes) {
	...
	for (int i = 0; i < m->key_values.len; i++) {
		...
		multi_return_u32_u32 mr_13500 = map_key_to_index(m, pkey);
		u32 index = mr_13500.arg0;
		u32 meta = mr_13500.arg1;
		multi_return_u32_u32 mr_13538 = map_meta_less(m, index, meta);
		index = mr_13538.arg0;
		meta = mr_13538.arg1;
...
VV_LOCAL_SYMBOL void map_cached_rehash(map* m, u32 old_cap) {
	...
	for (u32 i = ((u32)(0U)); i <= (u32)(old_cap + old_extra_metas); i += 2U) {
		...
		multi_return_u32_u32 mr_14337 = map_meta_less(m, index, meta);
		index = mr_14337.arg0;
		meta = mr_14337.arg1;
...
VV_LOCAL_SYMBOL voidptr map_get_and_set(map* m, voidptr key, voidptr zero) {
	for (;;) {
		multi_return_u32_u32 mr_14785 = map_key_to_index(m, key);
		u32 index = mr_14785.arg0;
		u32 meta = mr_14785.arg1;
		for (;;) {
			if (meta == m->metas[index]) {
				int kv_index = ((int)(m->metas[(u32)(index + 1U)]));
				voidptr pkey = DenseArray_key(&m->key_values, kv_index);
...
VV_LOCAL_SYMBOL voidptr map_get(map* m, voidptr key, voidptr zero) {
	multi_return_u32_u32 mr_15512 = map_key_to_index(m, key);
	u32 index = mr_15512.arg0;
	u32 meta = mr_15512.arg1;
	for (;;) {
		if (meta == m->metas[index]) {
			int kv_index = ((int)(m->metas[(u32)(index + 1U)]));
			voidptr pkey = DenseArray_key(&m->key_values, kv_index);
...
VV_LOCAL_SYMBOL voidptr map_get_check(map* m, voidptr key) {
	multi_return_u32_u32 mr_16177 = map_key_to_index(m, key);
	u32 index = mr_16177.arg0;
	u32 meta = mr_16177.arg1;
	for (;;) {
		if (meta == m->metas[index]) {
			int kv_index = ((int)(m->metas[(u32)(index + 1U)]));
			voidptr pkey = DenseArray_key(&m->key_values, kv_index);
...
VV_LOCAL_SYMBOL bool map_exists(map* m, voidptr key) {
	multi_return_u32_u32 mr_16687 = map_key_to_index(m, key);
	u32 index = mr_16687.arg0;
	u32 meta = mr_16687.arg1;
	for (;;) {
		if (meta == m->metas[index]) {
			int kv_index = ((int)(m->metas[(u32)(index + 1U)]));
			voidptr pkey = DenseArray_key(&m->key_values, kv_index);
...
// Attr: [unsafe]
void map_delete(map* m, voidptr key) {
	multi_return_u32_u32 mr_17325 = map_key_to_index(m, key);
	u32 index = mr_17325.arg0;
	u32 meta = mr_17325.arg1;
	multi_return_u32_u32 mr_17361 = map_meta_less(m, index, meta);
	index = mr_17361.arg0;
	meta = mr_17361.arg1;
	for (;;) {
		if (!(meta == m->metas[index])) break;
...
string string_replace_each(string s, Array_string vals) {
	...
	if (idxs.len > 0) { qsort(idxs.data, idxs.len, idxs.element_size, (int (*)(const void *, const void *))&compare_16615326395714869970_RepIndex_by_idx); };
	u8* b = malloc_noscan((int)(new_len + 1));
	int idx_pos = 0;
...
string string_trim(string s, string cutset) {
	...
	multi_return_int_int mr_45121 = string_trim_indexes(s, cutset);
	int left = mr_45121.arg0;
	int right = mr_45121.arg1;
...
VV_LOCAL_SYMBOL int os__vpclose(voidptr f) {
	...
	#else
	{
		multi_return_int_bool mr_9447 = os__posix_wait4_to_exit_status(pclose(f));
		int ret = mr_9447.arg0;
		return ret;
	}
	#endif
	return 0;
...
int os__system(string cmd) {
	...
	#if !defined(_WIN32)
	{
		multi_return_int_bool mr_10491 = os__posix_wait4_to_exit_status(ret);
		int pret = mr_10491.arg0;
		bool is_signaled = mr_10491.arg1;
		ret = pret;
		if (is_signaled) {
...
_result_Array_string os__glob(Array_string patterns) {
 ...
 ;
	...
	if (matches.len > 0) { qsort(matches.data, matches.len, matches.element_size, (int (*)(const void *, const void *))&compare_3722393162571173239_string); };
...
VV_LOCAL_SYMBOL void main__main(void) {
	...
	for (int _t1 = 0; _t1 < args.len; ++_t1) {
		...
		if (string_contains(arg, _SLIT("--file"))) {
			string s = string_replace(arg, _SLIT("--file="), _SLIT(""));
			array_push((array*)&files, _MOV((string[]){ string_clone(s) }));
		}
	}
	{
	}
	for (int _t3 = 0; _t3 < files.len; ++_t3) {
		...
		string s = os__abs_path(f);
		if (os__exists(s) && os__is_file(s)) {
			_result_Array_string _t4 = os__read_lines(s);
			if (_t4.is_error) {
				IError err = _t4.err;
 			...
 			Array_string data =  (*(Array_string*)_t4.data);
			int bracket_count = 0;
			bool match_start = false;
			Array_string out_file = __new_array_with_default(0, 0, sizeof(string), 0);
			for (int _t5 = 0; _t5 < data.len; ++_t5) {
				string i = ((string*)data.data)[_t5];
				if (string_contains(i, _SLIT("match"))) {
					match_start = true;
				}
				if (match_start) {
					for (int _t6 = 0; _t6 < i.len; ++_t6) {
						u8 c = i.str[_t6];
						if (c == *"{") {
							bracket_count = (int)(bracket_count + 1);
						} else if (c == *"}") {
							bracket_count = (int)(bracket_count - 1);
						}
					}
					...
					if (match_start) {
						if (string_contains(i, _SLIT("=>"))) {
							string str = string_replace(i, _SLIT(" "), _SLIT(""));
							string ch = string_substr(str, (int)(str.len - 1), str.len);
							_option_int _t7 = string_index(str, _SLIT("=>"));
							if (_t7.state != 0) {
								IError err = _t7.err;
								println(IError_str(err));
								return;
 							...
 							int ch_i =  (*(int*)_t7.data);
							string ch2 = string_substr(str, (int)(ch_i + 2), (int)(ch_i + 3));
							if (string__eq(ch, _SLIT(","))) {
								string l1 = string_replace(i, _SLIT("=>"), _SLIT("{"));
								string l2 = string_replace(l1, _SLIT(","), _SLIT("}"));
								array_push((array*)&out_file, _MOV((string[]){ string_clone(l2) }));
								continue;
							} else if (!string__eq(ch2, _SLIT("{"))) {
								string l1 = string_replace(i, _SLIT("=>"), _SLIT("{"));
								string l2 = string__plus(l1, _SLIT("}"));
								array_push((array*)&out_file, _MOV((string[]){ string_clone(l2) }));
								continue;
							} else {
								string l1 = string_replace(i, _SLIT("=>"), _SLIT(""));
								array_push((array*)&out_file, _MOV((string[]){ string_clone(l1) }));
								continue;
							}
						}
					}
				}
				array_push((array*)&out_file, _MOV((string[]){ string_clone(i) }));
			}
			_result_void _t12 = os__write_lines(f, out_file);
			if (_t12.is_error) {
				IError err = _t12.err;
				println(IError_str(err));
				return;
